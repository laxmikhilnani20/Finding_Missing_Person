...
async function addCamera() {
    ...
    const ip_address = document.getElementById('camera-ip').value.trim();
    const port = parseInt(document.getElementById('camera-port').value) || 4747;

    if (!name || !ip_address) {
        showAlert('Please enter camera name and IP address', 'warning');
        return;
    }

    const result = await apiCall('/api/cameras/add', 'POST', { name, ip_address, port });
    ...
    if (result.success) {
        ...
        document.getElementById('camera-port').value = '4747';
        loadCameras();
    } else {
...
async function testCamera() {
    ...
    if (!ip_address) {
        showAlert('Please enter DroidCam IP address', 'warning');
        return;
    }

    showAlert('Testing connection...', 'info');
    const result = await apiCall('/api/cameras/test', 'POST', { ip_address, port });
