...
def initialize_system():
    ...
    if not app_state.initialized:
        ...
        app_state.camera_manager = DroidCamManager()
        app_state.initialized = True
        
        # Load saved camera configuration
        saved_cameras = app_state.db_manager.load_camera_config()
        for cam in saved_cameras:
            # Support both DroidCam and legacy IP camera formats
            if 'ip_address' in cam:
                # DroidCam format
                port = cam.get('port', 4747)
                app_state.camera_manager.add_camera(
                    cam['id'], cam['name'], cam['ip_address'], port
                )
            elif 'url' in cam:
                # Legacy IP camera format - convert to DroidCam IP if possible
                print(f"⚠️ Legacy IP camera format detected for {cam['name']}. Please update to DroidCam format.")
...
def add_camera():
    """Add new DroidCam camera"""
    data = request.json
    cam_name = data.get('name')
    cam_ip = data.get('ip_address')
    cam_port = data.get('port', 4747)
    
    if not cam_name or not cam_ip:
        return jsonify({'success': False, 'message': 'Name and IP address required'})
    
    cam_id = f"droid_cam_{int(time.time())}"
    if app_state.camera_manager.add_camera(cam_id, cam_name, cam_ip, cam_port):
        # Save configuration
        cameras = app_state.camera_manager.get_all_camera_info()
        camera_list = [
            {'id': cid, 'name': info['name'], 'ip_address': info['ip_address'], 'port': info['port']}
...
def test_camera():
    ...
    if app_state.camera_manager.test_connection(cam_ip, cam_port):
